...
async function getStudentsData(content, page) {
   try{ 
   let res = await axios.get(
    ...
    pagination.innerHTML +=`
                    <li>
                        ...
                        data-page="${page - 1}"
                        class="page-link ${page === 1 ? "hidden" : ""} flex items-center justify-center text-body text-fg-brand rounded-l-md text-body bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:bg-neutral-tertiary-medium hover:text-heading font-medium rounded-s-base text-sm px-3 h-9">
                                    Previous
                        </a>
                    </li>`;
    for(let i = 1 ; i<= pages ; i++){
        pagination.innerHTML += `
                    <li>                    
                         <a href="javascript:void(0)"
                            data-page="${i}"
                            class="page-link ${page === i ? "dark:bg-gray-900 bg-black-300" : ""} flex items-center justify-center text-body text-fg-brand dark:text-gray-400 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:bg-neutral-tertiary-medium hover:text-heading text-sm w-10 h-9">
                                                                ${i}
                        </a>
                    </li>`
    }
    pagination.innerHTML +=`
                    <li>
                        <a href="javascript:void(0)"
                        data-page="${page + 1}"
                        class="page-link ${page === pages ? "hidden" : ""} flex items-center justify-center text-body text-fg-brand rounded-r-md text-body bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:bg-neutral-tertiary-medium hover:text-heading font-medium rounded-e-base text-sm px-3 h-9">
                                                                Next
                        </a>
                    </li>`
    ...
    res.data.forEach((el) => {        
        content.innerHTML += `
        <div class="student-card text-card-foreground flex flex-col gap-6 rounded-xl border p-6 cursor-pointer transition-all duration-300 hover:shadow-xl hover:-translate-y-1 bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 group" data-id="${el.id}">
            <div class="flex flex-col items-center text-center mb-4">
                <span data-slot="avatar" class="relative flex size-10 shrink-0 overflow-hidden rounded-full h-20 w-20 mb-3 ring-4 ring-purple-100 dark:ring-purple-900">
                    <img data-slot="avatar-image" class="aspect-square size-full object-cover"
                        alt="${el.name}"
                        src="${el.avatar}"
                        onerror="this.src='https://ui-avatars.com/api/?name=${encodeURIComponent(el.name)}&background=6366f1&color=fff'">
                </span>
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-1">${el.name}</h3>
                <div class="flex items-center gap-2 mb-3">
                    ...
                    <span class="text-gray-600 dark:text-gray-400 text-sm">${el.age} years old</span>
                </div>
                <div class="w-full space-y-2 mb-3">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-1">
                            ...
                            <span class="text-sm font-medium text-gray-900 dark:text-white">${el.rating}%</span>
                        </div>
                        <div class="flex items-center gap-1">
                            ...
                            <span class="text-sm font-medium text-gray-900 dark:text-white">${el.coin}</span>
                        </div>
                    </div>
                    <div class="bg-gray-200 dark:bg-gray-700 w-full overflow-hidden rounded-full h-2">
                        <div class="bg-gradient-to-r from-blue-500 to-purple-600 h-full transition-all" style="width: ${Math.min(el.rating, 100)}%;"></div>
                    </div>
                </div>
            </div>
            ...
            <div class="flex gap-3 mt-auto opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <button data-action="edit" data-id="${el.id}" class="flex-1 flex items-center justify-center gap-2 h-10 rounded-md bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 text-sm font-medium text-gray-800 dark:text-gray-200 transition">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                    </svg>
                    Edit
                </button>
                <button data-action="delete" data-id="${el.id}" class="flex-1 flex items-center justify-center gap-2 h-10 rounded-md bg-red-100 hover:bg-red-200 text-red-600 dark:bg-red-900/40 dark:hover:bg-red-900/60 dark:text-red-300 text-sm font-medium transition">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
getStudentsData(content, page)

function changePage(i){
    if (i < 1) return;
    page = i;
    getStudentsData(content, i);
}
